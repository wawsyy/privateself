...
const CONTRACT_NAME = "DreamJournal";
...
// Sepolia deployment (required for production)
let deploySepolia = readDeployment("sepolia", 11155111, CONTRACT_NAME, true /* optional */);

// Localhost deployment (optional, fallback to sepolia if not available)
let deployLocalhost = readDeployment("localhost", 31337, CONTRACT_NAME, true /* optional */);

// If localhost doesn't exist, use sepolia ABI and a placeholder address
if (!deployLocalhost) {
  if (deploySepolia) {
    deployLocalhost = { abi: deploySepolia.abi, address: "0x0000000000000000000000000000000000000000" };
  } else {
    console.error(`${line}No deployment found. Please deploy to either localhost or sepolia.${line}`);
    process.exit(1);
  }
}

// If sepolia doesn't exist, use localhost ABI and a placeholder address
if (!deploySepolia) {
